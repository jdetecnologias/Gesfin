...
$pdo = new PDO($sqlite);
$execucao = $pdo->prepare("select s.descricao as stat,t.descricao as tipo,c.id,c.descricao,c.valor,c.data_venc,c.data_pgto from contas as c inner join tipo as t on (c.tipo = t.id) inner join status as s on( s.id = c.status) where mes = :mes and user = :usuario and ano = :ano order by c.data_venc");
